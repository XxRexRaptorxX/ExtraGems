    ...
    public void stepOn(Level level, BlockPos pos, BlockState state, Entity entity) {
        if (!level.isClientSide()) {
            if (entity instanceof Player) {
                ...
                player.addEffect(new MobEffectInstance(GemHelper.getEffect(BuiltInRegistries.BLOCK.getKey(this).toString()), Config.CHARGED_BLOCK_EFFECT_DURATION.get(),
                        Config.CHARGED_BLOCK_EFFECT_AMPLIFIER.get()));
            }
            if (entity instanceof LivingEntity) {
                LivingEntity mob = (LivingEntity) entity;
                mob.addEffect(new MobEffectInstance(GemHelper.getEffect(BuiltInRegistries.BLOCK.getKey(this).toString()), Config.CHARGED_BLOCK_EFFECT_DURATION.get(),
                        Config.CHARGED_BLOCK_EFFECT_AMPLIFIER.get()));
            }
        }
    }
